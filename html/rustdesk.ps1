...
function get-GithubRelease {
    ...
    # this is the URL to the latest release
    # (Invoke-RestMethod https://api.github.com/repos/rustdesk/rustdesk/releases/tags/nightly).assets.browser_download_url
    ...
    if ($NoGui) {
        ...
        $DownloadSelection = @{}
        $extraModules = @('Microsoft.PowerShell.ConsoleGuiTools')
        ModulePrerequisites -Modules $extraModules -install
        $OSFilter = [ordered]@{
            'Windows' = '64.exe'
            'Linux'   = '64.deb'
        }
        if ($NoFilter) {
            $DownloadList.File | Out-ConsoleGridView -Title "Select the files to download" -OutputMode Single -OutVariable DownloadSelection 
        }
        else {
            $OSChoice = $OSFilter.Keys | Out-ConsoleGridView -Title "Select an OS" -OutputMode Single -OutVariable OSChoice
            $DownloadList.File | Out-ConsoleGridView -Title "Select the files to download" -OutputMode Multiple -Filter $OSFilter[$OSChoice] -OutVariable DownloadSelection
        }
    }
}
$DownloadList 
