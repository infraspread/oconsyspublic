...
function CHECK_CONFIG() {
    ...
    CONFIGCHECKRESULT="$(source /opt/venvs/matrix-synapse/bin/activate && python3 -m synapse.config -c /etc/matrix-synapse/conf.d/homeserver.yaml 2>&1)"
    if [[ "$CONFIGCHECKRESULT" = *"parses OK"* ]]; then
        Check_Config="${CONFIGCHECKRESULT}"
    else
        Check_Config="Syntax Error"
    fi
}

function CHECK_STATUS() {
    echo_white 'Checking Service Status...'
    Status="Service Status: $(systemctl is-active matrix-synapse.service)"
}
...
function CREATE_DB() {
    ...
    MATRIXDB_MARKER=$(grep -c '###MATRIX_' $PGHBA)
    ...
    sudo -u postgres psql -c "CREATE DATABASE $dbname WITH TEMPLATE = template0 ENCODING = 'UTF8' LOCALE_PROVIDER = libc LOCALE = 'C' OWNER $dbuser;"
}
...
function DIALOG_INPUTVALUE() {
    ORIGINAL_VALUE="$1"
    DESCRIPTION="$2"
    NEW_VALUE=$(\
        dialog --keep-tite --title "Change ${ORIGINAL_VALUE}" \
        --inputbox "${DESCRIPTION}" 8 40 \
        3>&1 1>&2 2>&3 3>&- \
    )
    return $NEW_VALUE
}
...
function DATABASE_MENU() {
    VAR_MENU=($(awk -F'=' '/^[a-z_][a-zA-Z0-9_]*=/ {print $1}' $SCRIPT_FULL_PATH))
    
    clear
    echo_red '*********************************************'
    echo_red '* Infraspread UG - Matrix Synapse Installer *'
    echo_red '*********************************************'
    echo
    
    function ADD_MENUELEMENT() {
        let NEXT_POS="${#VAR_MENU[*]}+1"
        VAR_MENU[$NEXT_POS]="$1"
    }
    
    ADD_MENUELEMENT "CreateDB"
    ADD_MENUELEMENT "DropDB"
    ADD_MENUELEMENT "Continue"
    ADD_MENUELEMENT "Quit"
    
    I=1
    for VARIABLE in "${VAR_MENU[@]}"; do
        VALUE=$(echo $VARIABLE | tr -d ' ')
        echo -e $WHITE$I $GREEN$VARIABLE $CYAN${!VALUE}
        let "I=I+1"
...
function MENU() {
    ...
    ADD_MENUELEMENT "Database_Menu"
    ADD_MENUELEMENT "TestFn"
    ADD_MENUELEMENT "NewUser"
    ADD_MENUELEMENT "Status"
    ADD_MENUELEMENT "Check_Config"
    ADD_MENUELEMENT "ServiceRestart"
    ADD_MENUELEMENT "Service_Log"
    ADD_MENUELEMENT "CreateDB"
    ADD_MENUELEMENT "Daemon_Log"
    ADD_MENUELEMENT "Remove"
    ADD_MENUELEMENT "Continue"
    ADD_MENUELEMENT "Quit"
        ...
        case $opt in
            *)
                if [ $opt == "Continue" ]; then
                    ...
                    break
                    elif [ $opt == "Check_Config" ]; then
                    CHECK_CONFIG
                    break
                    ###
                    elif [ $opt == "TestFn" ]; then
                    echo "Test Function"
                    $TestFnDlg=$(dialog --keep-tite --title "Test Function" --inputmenu "Enter new values" 10 40 8 "one" 2 "two" 3 "three")
                    read "Press [Enter] key to continue..."
                    break
                    ###
                    elif [ $opt == "Database_Menu" ]; then
                    echo "Database Menu"
                    DATABASE_MENU
                    break
                    ...
                    register_new_matrix_user -c /etc/matrix-synapse/conf.d/homeserver.yaml http://localhost:${matrix_port}
                    break
                    elif [ $opt == "CreateDB" ]; then
                    CREATE_DB
                    break
                    ...
                    #echo "current value: "${!opt}
                    #read new_value
                    DIALOG_INPUTVALUE "${opt}" "Enter new value for ${opt}\ncurrent: ${!opt}"
                    eval $opt=${NEW_VALUE}
                    break
                fi
            ;;
...
CREATE_CONFIG
cp $CONFIG_FULLNAME $CONFIG_PATH/..
