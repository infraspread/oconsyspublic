...
WAN=("IP Autoresolve" "DNS/Domain" "manual")
...
;;
"manual")
echo "enter IP or DNS for Rustdesk Server"
read wanip
break
;;
...
string="{\"host\":\"${wanip}\",\"relay\":\"${wanip}\",\"api\":\"https://${wanip}\",\"key\":\"${key}\"}"
string64=$(echo -n "$string" | base64 -w 0 | tr -d '=')
rustconf=$(echo -n "$string64" | rev)

# Create windows install script
wget https://raw.githubusercontent.com/infraspread/oconsyspublic/main/html/rustdesk_wininst.ps1
sudo sed -i "s|wanipreg|${wanip}|g" rustdesk_wininst.ps1
sudo sed -i "s|rustconf|${rustconf}|g" rustdesk_wininst.ps1
sudo sed -i "s|keyreg|${key}|g" rustdesk_wininst.ps1
...
mv /opt/rustdesk/rustdesk_wininst.ps1 /opt/gohttp/public/
...
Type=simple
LimitNOFILE=1000000
ExecStart=/opt/gohttp/gohttpserver -r ./public --port ${DLPORT} --auth-type http --auth-http admin:${DLPASS}
...
echo -e "You can access your install scripts for clients by going to http://${wanip}:${DLPORT}"
